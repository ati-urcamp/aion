<script>
  ...
  import TarefaSituacaoApiService from '../../../services/domains/TarefaSituacao'
  ...
  // Services
  const tarefaSituacaoService = new TarefaSituacaoApiService()
  ...
  export default {
    ...
    methods: {
      async loadItem (id) {
        if (id) {
          this.isEdit = true
          tarefaSituacaoService.get({ id: id })
            .then(response => {
              this.form = { ...response }
      ...
      onSave () {
        ...
        tarefaSituacaoService.save({ data: data })
          .then(() => {
            notifyService.success({ hideLoader: true })
            this.isSubmitted = false
            this.$router.push({ name: 'tarefa-situacao.index' })
          })
          .catch(e => {
            console.log(e)
            this.isSubmitted = false
...
<template>
    <div class="page-content">
                ...
                <v-form :model="form">
                    <v-container grid-list-md>
                        <v-layout row wrap>
                            ...
                            <v-flex xs12>
                                <v-switch v-model="form.fl_visivel"
                                          :error-messages="validationMessageField($v.form.fl_visivel)"
                                          label="Visível no board"></v-switch>
                            </v-flex>
                            <v-flex xs12 class="pa-0">
                                <v-flex xs12>
        ...
        <form-save-actions :event-click="onSave"
                           ...
                           route-back-name="tarefa-situacao.index"/>

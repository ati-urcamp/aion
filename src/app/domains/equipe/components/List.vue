<script>
  ...
  export default {
    components: { AppPageHeader, ListMain, ListEditButton, ListDeleteButton },
    data () {
      return {
        isLoadingData: true,
        filters: {
        ...
        dependencies: {},
        defaultSort: [{ column: 'arvore_humanizada', direction: 'ASC' }]
      }
    },
    async mounted () {
      this.listStore.listSort = this.defaultSort

      bus.$emit('list-init', { domain: 'equipe' }, () => {
    ...
    methods: {
      onFilter () {
        ...
        if (this.filters.nome) {
          searchData.arvore_humanizada = { 'ilike': '%' + this.filters.nome + '%' }
        }
...
<template>
    <div class="page-content">
        ...
        <app-page-header store-route-name="equipe.create"/>
                ...
                <div class="v-table__overflow">
                    <table class="v-datatable v-table theme--light">
                        ...
                        <tr>
                            ...
                            <th class="column">Encarregado</th>
                        ...
                        <tr v-for="item in listStore.listData" :key="`item-${item.id}`">
                            <td>{{item.id}}</td>
                            <td>{{item.arvore_humanizada}}</td>
                            <td>{{item.email}}</td>
                            <td>{{item.encarregado}}</td>
                            <td class="list-table-nowrap">
                                <div class="btn-actions">
                                    <ListEditButton route-name="equipe.edit" :item="item" domain="equipe"/>
                                    <ListDeleteButton :item="item" domain="equipe"/>
                                </div>
                            </td>
                        </tr>
                        </tbody>
